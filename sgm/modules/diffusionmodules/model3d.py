...
class AttnBlock(nn.Module):
    ...
    def attention(self, h_: torch.Tensor) -> torch.Tensor:
        ...
        b, c, h, w, d = q.shape
        q, k, v = map(
            lambda x: rearrange(x, "b c h w d -> b 1 (h w d) c").contiguous(), (q, k, v)
        )
        ...
        return rearrange(h_, "b 1 (h w d) c -> b c h w d", h=h, w=w, d=d, c=c, b=b)
...
class MemoryEfficientAttnBlock(nn.Module):
    ...
    def attention(self, h_: torch.Tensor) -> torch.Tensor:
        ...
        # compute attention
        B, C, H, W, D = q.shape
        q, k, v = map(lambda x: rearrange(x, "b c h w  d-> b (h w d) c"), (q, k, v))
        ...
        return rearrange(out, "b (h w d) c -> b c h w d", b=B, h=H, w=W, c=C, d=D)
...
class MemoryEfficientCrossAttentionWrapper(MemoryEfficientCrossAttention):
    def forward(self, x, context=None, mask=None, **unused_kwargs):
        ...
        out = rearrange(out, "b (h w d) c -> b c h w d", h=h, w=w, d=d, c=c)
        return x + out
