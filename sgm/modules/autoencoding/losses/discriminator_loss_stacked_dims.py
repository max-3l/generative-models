...
class GeneralLPIPSWithDiscriminator(nn.Module):
    def __init__(
        ...
        channels_to_dims: Optional[bool] = False,
        lpips_on_x_dims: Optional[int] = -1,
        lpips_dim_distribution: Optional[str] = "uniform",
    ):
        super().__init__()
        self.dims = dims
        self.channels_to_dims = channels_to_dims
        self.lpips_on_x_dims = lpips_on_x_dims
        if self.lpips_on_x_dims != -1:
            if lpips_dim_distribution == "uniform":
                self.lpips_dim_distribution = np.ones(self.dims) / self.dims
            elif lpips_dim_distribution == "linear":
                self.lpips_dim_distribution = np.linspace(0, 1, self.dims)
            elif lpips_dim_distribution == "exponential":
                self.lpips_dim_distribution = np.exp(np.linspace(0, 1, self.dims))
            elif lpips_dim_distribution == "normal":
                self.lpips_dim_distribution = np.random.normal(
                    0, 1, self.dims
                )
            else:
                raise ValueError(
                    f"Unknown distribution {lpips_dim_distribution} for lpips_dim_distribution"
                )
    ...
    def forward(
        ...
        *,  # added because I changed the order here
        ...
        if self.channels_to_dims:
            inputs, reconstructions = map(
                lambda x: x.unsqueeze(1), (inputs, reconstructions))
        ...
        if self.perceptual_weight > 0:
            if self.lpips_on_x_dims != -1:
                selected_inputs, selected_reconstructions = map(
                    lambda x: rearrange(x, "(b t) c h w -> b c t h w", t=self.dims),
                    (inputs, reconstructions),
                )
                assert self.lpips_on_x_dims < selected_inputs.shape[2]
                # Random sample from linear distribution over dim ot select lpips_on_x_dims different values
                # Withtout repition
                selected_dims = torch.from_numpy(np.random.choice(
                    self.dims, self.lpips_on_x_dims, replace=False, p=self.lpips_dim_distribution
                ))
                selected_inputs, selected_reconstructions = map(
                    lambda x: rearrange(x[:, :, selected_dims], "b c t h w -> (b t) c h w"), (selected_inputs, selected_reconstructions)
                )
                p_loss = self.perceptual_loss(
                    selected_inputs.contiguous(), selected_reconstructions.contiguous()
                )
                p_loss = rearrange(p_loss, "(b t) c h w -> b c t h w", t=self.lpips_on_x_dims).mean(dim=2, keepdim=True)
                rec_loss = rearrange(rec_loss, "(b t) c h w -> b c t h w", t=self.dims)
                rec_loss = rec_loss + self.perceptual_weight * p_loss
                rec_loss = rearrange(rec_loss, "b c t h w -> (b t) c h w")
            else:
                selected_inputs, selected_reconstructions = inputs, reconstructions
                p_loss = self.perceptual_loss(
                    selected_inputs.contiguous(), selected_reconstructions.contiguous()
                )
                rec_loss = rec_loss + self.perceptual_weight * p_loss
