...
class SwinUNETR(nn.Module):
    ...
    def forward(self, x_in):
        ...
        enc0 = checkpoint.checkpoint(self.encoder1, x_in)
        enc1 = checkpoint.checkpoint(self.encoder2, hidden_states_out[0])
        enc2 = checkpoint.checkpoint(self.encoder3, hidden_states_out[1])
        enc3 = checkpoint.checkpoint(self.encoder4, hidden_states_out[2])
        dec4 = checkpoint.checkpoint(self.encoder10, hidden_states_out[4])
        ...
        dec3 = checkpoint.checkpoint(self.decoder5, dec4, hidden_states_out[3])
        dec2 = checkpoint.checkpoint(self.decoder4, dec3, enc3)
        dec1 = checkpoint.checkpoint(self.decoder3, dec2, enc2)
        dec0 = checkpoint.checkpoint(self.decoder2, dec1, enc1)
        out = checkpoint.checkpoint(self.decoder1, dec0, enc0)
    ...
    def encode_stages(self, x_in, num_stages=6) -> list:
        assert 0 < num_stages < 7, "num_stages should be between 1 and 6 (inclusive)."
        hidden_states_out = self.swinViT(x_in, self.normalize)
        if num_stages == 1:
            return [hidden_states_out]
        enc0 = self.encoder1(x_in)
        if num_stages == 2:
            return [hidden_states_out, enc0]
        enc1 = self.encoder2(hidden_states_out[0])
        if num_stages == 3:
            return [hidden_states_out, enc0, enc1]
        enc2 = self.encoder3(hidden_states_out[1])
        if num_stages == 4:
            return [hidden_states_out, enc0, enc1, enc2]
...
class SwinTransformer(nn.Module):
    ...
    def encode_stages(self, x, num_stages, normalize=True):
        assert 1 <= num_stages <= 5, "num_stages should be 1 to 5"
...
class UniversalUNETR(SwinUNETR):
