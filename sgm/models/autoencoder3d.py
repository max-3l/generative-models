...
class AutoencodingEngine(AbstractAutoencoder):
    ...
    def forward(
        ...
        dec = self.decode(z.to(x.dtype), **additional_decode_kwargs)
...
class AEIntegerWrapper(nn.Module):
    ...
    def decode(
        ...
        # expect inds shape (b, s) with s = h*w*d
        shape = default(shape, self.shape)  # Optional[(h, w, d)]
        if shape is not None:
            assert len(shape) == 2, f"Unhandeled shape {shape}"
            inds = rearrange(inds, "b (h w d) -> b h w d", h=shape[0], w=shape[1], d=shape[2])
        h = self.regularization.get_codebook_entry(inds)  # (b, h, w, d, c)
        h = rearrange(h, "b h w d c -> b c h w d")
